...
class STAR(nn.Module):
    def __init__(self, gender="female", num_betas=10):
        ...
        self.register_buffer(
            "posedirs", torch.cuda.FloatTensor(star_model["posedirs"].reshape((-1, 93)))
        )
        # Mean Shape
        self.register_buffer(
        ...
        self.register_buffer(
            "shapedirs",
            torch.cuda.FloatTensor(np.array(star_model["shapedirs"][:, :, :num_betas])),
        )
        # Mesh traingles
        self.register_buffer(
    ...
    def forward(self, pose, betas, trans):
        ...
        v_posed = v_shaped + torch.matmul(posedirs, pose_feat[:, :, None]).view(
            -1, 6890, 3
        )
        ...
        G = G - rest
        T = (
            torch.matmul(self.weights, G.permute(1, 0, 2, 3).contiguous().view(24, -1))
        ...
        for i in range(0, self.parent.shape[0]):
            transform_i = with_zeros(
                torch.cat(
                    (
                        R[:, i + 1],
                        J[:, i + 1][:, :, None] - J[:, self.parent[i]][:, :, None],
                    ),
                    2,
                )
            )
