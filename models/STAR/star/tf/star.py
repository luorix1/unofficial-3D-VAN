...
def verts_core(pose, v, J, weights, kintree_table):
    ...
    rest_shape_h = tf.concat(
        [v, tf.ones([batch_size, 6890, 1], dtype=tf.float32)], axis=-1
    )
    v = (
...
class STAR(object):
    ...
    def __call__(self, pose, betas, trans):
        ...
        self.shapedirs = tf.constant(
            self.smpl_model["shapedirs"][:, :, : self.num_betas], dtype=dtype
        )
        ...
        tf_v_template = tf.constant(
            np.tile(self.smpl_model["v_template"], [batch_size, 1, 1]), dtype=dtype
        )
        v_shaped = tf.add(
        ...
        pose_feat = tf.concat(
            [
                quaternions_all(tf.reshape(pose, (-1, 24, 3)))[:, 4:],
                tf.expand_dims(betas[:, 1], axis=1),
            ],
            axis=1,
        )
