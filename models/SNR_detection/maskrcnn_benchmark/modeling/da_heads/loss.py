...
class DALossComputation(object):
    ...
    def prepare_masks(self, targets):
        ...
        for targets_per_image in targets:
            ...
            mask_per_image = (
                is_source.new_ones(1, dtype=torch.uint8)
                if is_source.any()
                else is_source.new_zeros(1, dtype=torch.uint8)
            )
            masks.append(mask_per_image)
        return masks

    def __call__(
        self, da_img, da_ins, da_img_consist, da_ins_consist, da_ins_labels, targets
    ):
        """
        Arguments:
            da_img (list[Tensor])
        ...
        for da_img_per_level in da_img:
            ...
            da_img_label_per_level = torch.zeros_like(
                da_img_per_level, dtype=torch.float32
            )
        ...
        da_consist_loss = consistency_loss(
            da_img_consist, da_ins_consist, da_ins_labels, size_average=True
        )
