...
def consistency_loss(img_feas, ins_fea, ins_labels, size_average=True):
    ...
    intervals = [
        torch.nonzero(ins_labels).size(0),
        len_ins - torch.nonzero(ins_labels).size(0),
    ]
    for img_fea_per_level in img_feas:
        ...
        assert (
            N == 2
        ), "only batch size=2 is supported for consistency loss now, received batch size: {}".format(
            N
        )
        for i in range(N):
