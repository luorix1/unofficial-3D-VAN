...
from models.AdaIN.AdaIN import adaptive_instance_normalization


# Define generic CNN decoder
...
def style_transfer(
    encoder, decoder, content, style, alpha=1.0, interpolation_weights=None
):
...
parser.add_argument("--encoder", type=str, default="tests/ckpt/vgg_normalised.pth")
parser.add_argument("--decoder", type=str, default="tests/ckpt/decoder.pth")
    ...
    if do_interpolation:  # one content image, N style image
        ...
        with torch.no_grad():
            output = style_transfer(
                encoder, decoder, content, style, args.alpha, interpolation_weights
            )
        output = output.cpu()
